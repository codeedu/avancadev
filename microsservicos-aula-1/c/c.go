...
func home(w http.ResponseWriter, r *http.Request) {
	...
	if valid == "valid" {
		// Check if coupon is valid and save it in order to not use it again
		resultCoupon := makeHttpCall("http://localhost:9093/coupons", coupon)

		if resultCoupon.Status == "added" {
			result.Status = "valid"
		}
